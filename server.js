...
app.get("/v1/items", async (req, res) => {
  ...
  const items = await pgClient.query(
    "SELECT * FROM items WHERE id = $1", [id]
      ...
      res.status(500).send(`Encountered an internal error when fetching item with ID ${id}`);
    });
  res.status(200).send(items.rows);
});
...
app.post("/v1/items", async (req, res) => {
  ...
  const item = await pgClient.query(
    "INSERT INTO items (id, item_name, complete) VALUES ($1, $2, $3)", [id, item_name, false]
    ).catch(e => {
      res.status(500).send("Encountered an internal error when creating an item");
      return
    });
...
app.put("/v1/items/:id", async (req, res) => {
  ...
  await pgClient.query(
    "UPDATE items SET item_name = $1, complete = $2 WHERE id = $3", [item_name, complete, id]
    ).catch(e => {
      res.status(500).send("Encountered an internal error when updating an item");
    });
